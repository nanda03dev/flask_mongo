...
from ..utils import response_util, file as file_util
...
def upload_book():
    if 'file' not in request.files:
        response_util.bad_request("file is mandatory")
    result = file_util.write_file(request.files["file"])
    if result["status"]:
        return response_util.success("File upload succesfully")
    else:
        return response_util.bad_request("Upload failed")
